...
public class RedisUtil {
  ...
  public Future<Response> getConfigValue(String rKey) {
    Future<Response> future = null;
    future = redisPool.getClientByIndex(0).get(rKey)
      .onSuccess(res -> {
        logger.info("getConfigValue " + rKey + "=" + res.toBuffer().toString());
      })
      .onFailure(res -> {
        logger.error("getConfigValue key=" + rKey, res.getCause());
      });
    return future;
  }
